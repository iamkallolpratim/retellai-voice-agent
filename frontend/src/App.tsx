...
import { TabType, AgentConfigCreate, CallRequest, Call, CallResult, AgentConfig } from './types';
...
const App: React.FC = () => {
  ...
  // Use custom hooks
  const { configs, loading: configsLoading, loadConfigs, createConfig, updateConfig, deleteConfig } = useAgentConfigs();
  const { calls, callResults, loading: callsLoading, loadCalls, createCall, refreshCallStatus, getCallResult } = useCalls();
  ...
  useEffect(() => {
    loadConfigs().catch(err => {
      showNotification('error', `Failed to load configurations: ${err.message}`);
    });
  }, []);
  ...
  useEffect(() => {
    const inProgressCalls = calls.filter(c => c.status === 'pending' || c.status === 'in_progress');
    
    if (inProgressCalls.length > 0 && activeTab === 'results') {
      const interval = setInterval(() => {
        inProgressCalls.forEach(call => {
          refreshCallStatus(call.id).catch(err => console.error('Failed to refresh:', err));
        });
      }, 3000);
  ...
  const handleSaveConfig = async () => {
    ...
    try {
      ...
      showNotification('error', `Failed to save configuration: ${error instanceof Error ? error.message : 'Unknown error'}`);
    } finally {
  ...
  const handleStartCall = async () => {
    if (!callRequest.driver_name || !callRequest.phone_number || !callRequest.load_number || !callRequest.agent_config_id) {
    ...
    try {
      ...
      showNotification('success', 'Call initiated successfully! Check results tab for status.');
      ...
      showNotification('error', `Failed to start call: ${error instanceof Error ? error.message : 'Unknown error'}`);
    } finally {
      setIsSubmitting(false);
    }
  };
    ...
    <div className="space-y-6">
      <div className="bg-white border-2 border-black p-6">
        <h2 className="text-2xl font-bold mb-4 flex items-center gap-2">
          ...
          {editingConfigId ? 'Edit Agent Configuration' : 'Create Agent Configuration'}
        </h2>
        <ConfigForm
  ...
  // Count active calls
  const activeCallsCount = calls.filter(c => c.status === 'in_progress' || c.status === 'pending').length;
    ...
    <div className="min-h-screen bg-gray-100">
      ...
      <header className="bg-black text-white p-6 border-b-4 border-white">
        <div className="max-w-7xl mx-auto">
          ...
          <p className="text-gray-300 mt-1">Configure, test, and review logistics voice agents</p>
        </div>
      </header>
        ...
        <div className="max-w-7xl mx-auto flex">
          <button
            ...
            className={`flex-1 py-4 px-6 font-bold transition-colors ${
              activeTab === 'config' ? 'bg-black text-white' : 'hover:bg-gray-100'
            }`}
          >
          ...
          <button
            ...
            className={`flex-1 py-4 px-6 font-bold transition-colors ${
              activeTab === 'results' ? 'bg-black text-white' : 'hover:bg-gray-100'
            }`}
          >
      ...
      <footer className="bg-black text-white text-center p-4 mt-12 border-t-4 border-white">
        <p className="text-sm">AI Voice Agent Tool - Logistics Management System</p>
